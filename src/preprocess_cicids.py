...
def process_cicids(folder_path, output_dir='models/cicids', test_size=0.30, val_split=0.5, random_state=42):
    ...
    # step 2: Fix column names (leading spaces)
    ...
    for col in numeric_columns:
        if np.isinf(X[col]).any():
            ...
            if pd.isna(max_val):  # only if all values are inf
                max_val = 0
    ...
    # First split: 70% train, 30% temp (val and test)
    ...
    try:
        ...
        # Oversample classes with custom targets
        smote_strategy = {
            1: 10000,
            3: 8000,
            5: 6000,
        ...
        # First undersample, then oversample
    ...
    # test data for dashboard simulation
